...
i(x) = m * (x ** 2) * log(x)
fit i(x) 'profile_mpi.dat' using 1:7 via m
# hstr = sprintf('x^{%.2g}', f)
...
plot 'profile.dat' using 1:5 title "Colin's OMP" lw 1.5, f(x) title fstr lw 1.5, '../serial/profile.dat' using 1:5 title "Serial" lw 1.5, g(x) title gstr lw 1.5, 'profile_serial.dat' using 1:5 title "Colin's Serial" lw 1.5, h(x) title hstr lw 1.5, 'profile_mpi.dat' using 1:7 title "MPI" lw 1.5, i(x) title istr lw 1.5
